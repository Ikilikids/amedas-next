...
const rankingFiles = [
  {
    path: "public/ranking/av_avtemp/bot/all.json",
    title: "年間平均気温",
    topN: 20,
  },
  {
    path: "public/ranking/lwtemp_0/top/all.json",
    title: "年間冬日日数",
    topN: 20,
  },
  {
    path: "public/ranking/hitemp_0/top/all.json",
    title: "年間真冬日日数",
    topN: 20,
  },
];
...
export async function getStaticProps() {
  ...
  const stationsRaw = selectedFiles.map((num) => {
    ...
    //割合データ
    const ssgArray1 = getValuesArrayFromData(data, "all", "lwtemp");
    const ssgArray2 = getValuesArrayFromData(data, "1", "lwtemp");
    const ssgArray3 = getValuesArrayFromData(data, "2", "lwtemp");
...
export default function StationPage({ station, rankingList }) {
  const DATA_OPTIONS = [
    ...
    { value: "chart1", label: "気温割合(1月)" },
    { value: "chart2", label: "気温割合(2月)" },
    { value: "map", label: "マップ" },
