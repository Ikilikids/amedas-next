...
const selectedFiles = ["31482", "42046"];

// ランキング
const rankingFiles = [
  {
    path: "public/ranking/sm_snowing/top/all.json",
    title: "年間降雪量",
    topN: 20,
  },
];
...
export async function getStaticProps() {
  ...
  const stationsRaw = selectedFiles.map((num) => {
    ...
    //割合データ
    const ssgArray1 = getValuesArrayFromData(data, "all", "snowing");
    const ssgArray2 = getValuesArrayFromData(data, "all", "snowed");
...
export default function StationPage({ station, rankingList }) {
  const DATA_OPTIONS = [
    { value: "chart0", label: "降雪割合(通年)" },
    { value: "chart1", label: "積雪割合(通年)" },
    { value: "map", label: "マップ" },
